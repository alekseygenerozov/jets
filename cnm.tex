...
As described in \citet{Generozov+2015}, we have analytic
expressions for the densities and temperatures of steady state
solutions to equation~\eqref{eq:model}. We will apply these analytics
across the physical range heating rates ($v_w$) and mass
injection rates ($\eta$), to obtain the physical range of CNM
gas densities.
...
radius of the jet was approximately $\sim 10^{18}$ cm. Thus, it is
convenient to approximate nuclear gas density as a power law:
$n(r)=n_{18} (r/10^{18})^{-k}$.
...
$n(\rs)$ depends on the injection rate of stellar wind material inside
of $\rs$, $\dot{M}$. This is simply related to the stellar mass
enclosed inside of $\rs$, $M_{\rm enc}(\rs)$:
...
density follows $\rho_\star\sim r^{-\delta_1}$, where $\delta_1 = 1-2$
...
{\bf AG need better transition here.}

% For radii inside of the break radius, but much larger than the
% stagnation radius, we expect that the gas density slope will be $\sim
% \delta-1$, for stellar density slope $\delta$.  
In the vicinity of the stagnation radius, we expect the gas density
slope to be $\sim (4\delta-1)/6$. The median $\delta$ for low mass
black holes ($\lsim 10^8 \Msun$) in the \citet{Lauer+2007} sample is
$\sim 1.7$, for which $n(r)\propto r^{-1}$ near $\rs$.  {\bf A bit
  more justification here...}  Thus, we adopt $n(r)= n_{18} (r/10^{18}
{\rm cm})^{-1}$, as our fiducial density profile, and $\delta=1.7$ as
our fiducial stellar density slope.  Combining
equations~\eqref{eq:dotM} and~\eqref{eq:nrs}, we obtain
