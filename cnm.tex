...
curve (see dash-dotted line in Fig.~\ref{fig:cores}). In general, we
do not expect CNM density profiles as steep as $n\propto r^{-1.5}$ on
the relevant scales (see $\S$\ref{sec:dProf}). For profiles shallower
than $n\propto r^{-1}$ we do not expect a significant difference in
the late time light curve slope, but this needs to be checked using
further 2D simulations.
...
\item The slope of the gas density profile depends on the slope of the
  ...
  not expect CNM density profiles that are this steep on the relevant
  radial scales (e.g. the deceleration radius) for jets with energy
  $\gsim 10^{53}$ erg and CNM densities of $n_{18}\sim 10$ cm$^{-3}$.
  For profiles shallower than $n\propto r^{-1}$ we do not expect a
