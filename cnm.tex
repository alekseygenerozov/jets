...
\begin{abstract}
  ...
  Radio follow ups several months to several years after the TDE
...
inferred for AGN jets or by modeling SwJ1644 (\citealt{Metzger+2012}),
and would require unphysically low jet half opening angle
$\theta_j\lsim 0.01$.
...
candidate.\footnote{There were radio detections for two ROSAT flares:
  ...
  luminosity is more consistent with superluminous supernova than a
...
in at the deceleration radius (after the jet has swept up a
...
winds.\footnote{Type II SNe are also an important heating source.  In
  a young stellar population, the power from Type II supernovae
  exceeds that from massive stellar winds after $\sim$6 Myr
...
maximum density of $n_{18} \sim 2,000\, \Mbh[,7]^{0.5}$ cm$^{-3}$ is achieved for a
...
jet would probably not result in the disruption of the jet. {\bf AG
  what about a star in the slower sheath--this is more likely!}
...
our standard estimates. Thus, we neglect mass drop out due to star
formation.
...
\includegraphics[width=8.5cm]{jetstruct.png}
...
\caption{\label{fig:jetstruct} Initial geometry of the jet used for
  ...
  emission. Note that for our 1D simulation we take a slow component
...
results. Note that for the for the optically thin case the peak time
...
passes through the observing band, and occurs later if the nuclear gas
density is higher. Otherwise, peak flux is achieved near the
deceleration time, which is a decreasing function of $n_{18}$
...
\begin{figure*} 
  ...
  \caption{\label{fig:lightcurves} \textit{Left:} Radio light curves
    ...
    \citet{Arcavi+2014} \textit{Right:} $5\times 10^{53}$ erg on-axis
    light curves from left column ({\it darker-shaded lines}) and
    corresponding slow component light curves ({\it lighter-shaded
      lines}).}
\end{figure*}
...
\caption{\label{fig:onOff} Comparison between on-axis (solid line) and
  ...
  r^{-1}$ for $n_{18}=2000$ cm$^{-3}$ (bottom panel), but take $n\propto
  r^{-1.5}$ for $n_{18}=60$ cm$^{-3}$ (top panel).  The latter
...
density model ($n_{18}=2$ cm$^{-3}$ and $E_j=5\times 10^{53}$
...
\begin{figure}
  ...
  \caption{\label{fig:reverse} Radio light curve from the forward
    ...
    $n\propto r^{-1}$ with $n_{18} = 2$ cm$^{-3}$. Both light curves
    are computed at a frequency of 5 GHz. The reverse shock light
    curve excludes synchrotron self-absorption, which when included in
    the full calculation results in large attenuation of the emission,
    such that the total light curve is dominated by the forward
    shock.}
\end{figure}
...
ambient gas density; this is in part coincidental, as the fast and
...
putative jet, can therefore be used to constrain the jet energy.
...
(\citealt{Mimica+2015}). 
%  The single detection {\bf AG Drake actually
%   has multiple detections} of CSS100217 requires an outflow energy of
% $1.7\times 10^{50}-6\times 10^{51}$ erg.  
...
SwJ2058 & 8300  & - & - & - & 3 & $5\times 10^{53}$\\ 
\hline 
Upper limits & \\
\hline
RXJ1624+7554 & 265.0 & 21.67 & 3.0 & 12 & 4 & $< 9.9 \times 10^{ 52 }$ \\
RXJ1242-1119 & 208.0 & 19.89 & 3.0 & 8 & 4 & $< 6.7 \times 10^{ 52 }$ \\
SDSSJ1323+48 & 365.0 & 8.61 & 3.0 & 48 & 4 & $< 1.0 \times 10^{ 53 }$ \\
SDSSJ1311-01 & 750.0 & 8.21 & 3.0 & 115 & 4 & $< 1.9 \times 10^{ 53 }$ \\
D1-9 & 1700.0 & 8.0 & 5.0 & 800 & 5 & $< 6.6 \times 10^{ 53 }$ \\
TDE1 & 645.0 & 5.4 & 5.0 & 120 & 5 & $< 1.0 \times 10^{ 53 }$ \\
D23H-1 & 910.0 & 4.8 & 5.0 & 200 & 5 & $< 1.3 \times 10^{ 53 }$ \\
PTF10iya & 1130.0 & 1.6 & 5.0 & 300 & 5 & $< 5.5 \times 10^{ 52 }$ \\
PS1-10jh & 820.0 & 0.71 & 5.0 & 300 & 5 & $< 2.3 \times 10^{ 52 }$ \\
NGC5905 & 52.0 & 21.91 & 3.0 & 2 & 4 & $< 2.6 \times 10^{ 52 }$ \\
NGC5905 & 75.0 & 6.0 & 8.6 & 9 & 6 & $< 1.4 \times 10^{ 52 }$ \\
D3-13 & 2000.0 & 7.6 & 5.0 & 960 & 5 & $< 7.2 \times 10^{ 53 }$ \\
D3-13 & 2000.0 & 1.8 & 1.4 & 1000 & 7 & $< 2.4 \times 10^{ 53 }$ \\
TDE2 & 1280.0 & 4.3 & 5.0 & 590 & 5 & $< 2.7 \times 10^{ 53 }$ \\
TDE2 & 1280.0 & 1.1 & 8.4 & 1650 & 8 & $< 1.1 \times 10^{ 53 }$ \\
SDSSJ1201+30 & 700.0 & 1.4 & 7.9 & 1000 & 9 & $< 1.0 \times 10^{ 53 }$ \\
PTF09axc & 540 & 5.0 & 3.5 & 670 & 10 & $< 3.9 \times 10^{ 53 }$ \\
PTF09axc & 540 & 5.0 & 6.1 & 530 & 10 & $< 2.7 \times 10^{ 53 }$ \\
\end{tabular*}
\begin{tablenotes}
\item References: $(1)$ \citet{Alexander+2015, van-Velzen+2015}, $(3)$
  \citet{Berger+2012, Zauderer+2013} , $(4)$ \citet{Cenko+2012}, $(5)$
  \citet{Bower+2013}, $(6)$ \citet{van-Velzen+2013}, $(7)$
  \citet{Bade+1996, Komossa&Dahlem2001}, $(8)$
...
\item We estimate the nuclear gas densities expected from injection of
  ...
  expected $n_{18}$ of $10 \Mbh[,7]^{-0.4}-100 \Mbh[,7]^{-0.4}$
  cm$^{-3}$ for host galaxies of most observed TDEs.
...
\item We take our two component jet model and run it through a range
  ...
  chosen value of $n_{18}$. The time of the peak luminosity depends,
  however, on the density and can be as early as months and as late as
  one decade after the TDE. By comparing radio detections and upper
  limits from a set optical/UV and soft X-ray selected TDE, we show
  that these sources cannot have jets as powerful as SwJ1644. Prompt
  follow-up in the radio band, as well as regular monitoring, would
  provide tighter constraints on the existence of TDE jets. Radio
  afterglows can serve as calorimeters for off-axis jets launched by
  TDEs, and future observational efforts that capture the peak radio
  flux in thermally detected TDEs will add to the diversity of jet
  energies observed in TDE flares.  The broad range of energies (both
  detections and upper limits) already seen in TDE jets presents an
  interesting puzzle for theoretical models of jet launching.
\end{enumerate}


\section*{Acknowledgments}
{\bf AG more acknowledgements here!}
PM and MAA acknowledge financial support from the European Research
Council (ERC) through the Starting Independent Researcher Grant
CAMAP-259276, and the partial support of grants AYA2013-40979-P,
