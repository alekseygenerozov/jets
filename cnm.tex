...
\section{Jet Models}
\label{sec:numerical}
We calculate radio light curves due to the jet shocking the CNM across
the physically plausible range of nuclear gas densities.  The jet-CNM
interaction simulated in one and two-dimensions, and the resulting
synchrotron emission is computed using the procedure described in
\citet{Mimica+2015}.  For the jet model we take the preferred
two-component model for SwJ1644 from \citet{Mimica+2015}: a fast,
$\Gamma=10$ inner core and a slow, $\Gamma=2$ outer sheath.


 We modify the geometry of the slow sheath for our 1D simulations to
 better reproduce the 2D results.  In \citet{Mimica+2015}, the fast
 inner core spans an angular interval $[0, 0.1\ {\rm rad}]$, while the
 slow outer sheath occupies $[0.1\ {\rm rad}, 0.5\ {\rm rad}]$.
 Although the sheath is injected in a relatively narrow angular
 interval, at late stages of the jet evolution the bow shock created
 by its interaction with CNM spans a much larger interval in 2D
 simulations i.e. the slow component becomes almost isotropic
 \citep[bottom two panels in Fig. 8 in][]{Mimica+2015}. To account for
 this, we take $[0.1 {\rm rad}, \,\pi/2\, {\rm rad}]$ for the angular
 extent of the slow component in our 1D simulations. The resulting
 light curves better match the results from the 2D jet simulation (see
 $\S~\ref{sec:1d2d}$). The ratio of the true energy of the fast
 component is fixed to be 4\% of the energy in the slow component, as
 in \citet{Mimica+2015}.
...
\end{figure*}
 ...
 extending from 0.1-$\pi/2$ radians for our 1D simulations
