...
The result in Figures~\ref{fig:1D2DB}-~\ref{fig:cores} only
include emission from the forward shock. In principal the reverse
shock may provide an additional contribution to the radio light
curve. However, as we describe in this section its contribution will
be negligible, at least for high energy jets.
...
the slow component ($\Gamma=2$) kinetic energy dissipated by
...
10^{53}$ erg jet propagation through our fiducial $n \propto r^{-1}$
density profile with density normalization $n_{18}$=2. Although the
emission from the reverse shock is comparable to that from the forward
shock at early times, the reverse shock emission contributes very
little to the total light curve, since it is absorbed at the front of
the jet. While the fraction of energy dissipated in the reverse shock
will be greater for a larger ambient density, any emission from the
reverse shock would be absorbed. Thus, the reverse
shock makes little difference for high energy ($E\sim 10^{53}$ erg)
jets.
...
\label{sec:reverse}
Her we estimate the fraction of jet kinetic energy that is dissipated by the reverse
shock, as opposed to the forward shock contribution focused on in this paper.

From continuity, the co-moving density of a steady relativistic jet is given by (e.g. \citealt{Uhm&Beloborodov07})
 \begin{align}
...
jet. The second term in the denominator is generally negligible if the jet Lorentz factor changes slowly
($\dot{\Gamma}_{\rm j} \ll c\Gamma^{3}/r$), as can be shown to be true at radii $r < r_{\rm dec}$ as long as $\Gamma$ changes on timescales $\gtrsim t_{\rm 0}$, where $t_{\rm 0}$ is the jet duration.

The Lorentz factor of the shocked material may be estimated using the relativistic shock jump condition and pressure equality between the forward and reverse shocks.  In the ultra-relativistic limit this gives,
\begin{equation}
...
\begin{equation}
  f\approx 40\,  L_{\rm j,48} n_{18}^{-1} \Gamma_{10}^{-2} \, \left(\frac{r}{10^{18} {\rm
        cm}}\right)^{-1} 
\end{equation}
is the ratio of the jet density to that of the CNM.  This expression is inaccurate for mildly relativistic or non-relativistic, in which case we apply the more general expression for $\Gamma_{\rm sh}$ given by 
...
\end{equation} 
%
From equations~\eqref{eq:gammaShGen} and ~\eqref{eq:betars} we calculate the radius and value of $\Gamma_{\rm sh}$ at the time when the reverse shock crosses the trailing edge of the jet.  This allows us to calculate the fraction of the initial kinetic energy of the jet which is dissipated at the reverse shock, instead of being immediately transferred to the shocked external medium via the forward shock.  
