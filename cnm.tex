...
approaching $n\propto r^{-1.5}$. The gas profile flattens to $n\propto
r^{1-\delta}$ between the stagnation radius and the stellar break
radius; however, for our fiducial value of $\delta = 1.7$, the
...
do not expect CNM density profiles as steep as $n\propto r^{-1.5}$
(see $\S$\ref{sec:dProf}). For profiles shallower than $n\propto
r^{-1}$ we do not expect a significant difference in the late time
light curve slope, but this needs to be checked using further 2D
simulations.
...
\item The slope of the gas density profile depends on the slope of the
  ...
  radial scales (e.g. the deceleration radius).  For profiles
