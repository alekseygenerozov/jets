...
but introduce a number of modifications regarding the jet geometry
...
In the two component model of \citet{Mimica+2015}, the fast inner core
spans an angular interval $[0, 0.1\ {\rm rad}]$, while the slow outer
sheath occupies $[0.1\ {\rm rad}, 0.5\ {\rm rad}]$. 
% For both
% components they assume $E_{\rm ISO} = 4\times 10^{54}$ erg. Note that,
% for fixed $E_{\rm ISO}$, the true jet energy depends only on the
% angular interval: $E_{\rm jet} = E_{\rm ISO} (\cos\theta_{\rm j,min} -
% \cos\theta_{\rm j,max})$.

Although the sheath is injected in a relatively narrow angular
interval, at late stages of the jet evolution the bow shock created by
its interaction with CNM spans a much larger interval in 2D
...
extent of the slow component for our 1D simulations. Doing so better
