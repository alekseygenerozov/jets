...
\begin{equation}
\n_{\rm Ia}=\frac{t_{\rm dyn}(r_{\rm Ia}) (\eta M_{\star} (r_{\rm
    Ia})/t_h)}{4 \pi/3 r_{\rm Ia}^3 m_p}
\label{eq:nIa}
\end{equation}

$t_{\rm dyn}(r_{\rm Ia})=\sqrt{\frac{G \Mbh}{r_{\rm Ia}^3}}$ is the
dynamical time at Ia radius. $M_{\star}(r_{\rm Ia})$ is the stellar
mass enclosed inside the Ia radius:

\begin{align}
&$\Mbh (r_{\rm Ia}/r_{inf})^{3-\delta}$\\
&$r_{\inf}\approx 3.5 \Mbh[7]$ pc
\end{align}
...
radius. Assuming $\n\propto r^{-1}$ we obtain $n_{18}\simeq 50$ cm$^{-3}$. 
