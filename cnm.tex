...
calculated for two different stellar density profiles. We mark the
...
$r_{\rm inf}$, where $M_{\star}(r_{\rm inf})=\Mbh.$ We take
$r_{\rm inf}=3.5 \Mbh[7]^{0.6}$. Thus, 

\begin{equation}
\dot{M}=
\end{equation}

% For our fiducial $\delta$ we take $\delta=1.7$, the median $\delta$
% for low mass ($\lsim 10^{8} \Msun$) black holes from the
% \citet{Lauer+2007} sample.
...
stellar density profile. In the vicinity of the stagnation radius, we
find that the slope of the gas density profile is $\sim
(4\delta-1)/6$. The median $\delta$ for low mass black holes ($\lsim
10^8 \Msun$) in the \citet{Lauer+2007} sample is $\sim 1.7$, which
gives $n(r)\propto r^{-1}$ near $\rs$. Also, the density profile
remains close to $n\propto r^{-1}$ for the relevant range of radii for
such a cuspy stellar density\footnote{For cusp galaxies, the gas
  velocity asymptotes to a constant between the stagnation radius and
  break radius (if they are sufficiently far apart). In this region we
...
population that is a Hubble time old plus a young component that
...
\Msun$) stars. Plugging this value of $r_s$ in equation~\eqref{eq:nrs}
