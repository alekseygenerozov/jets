...
$p=2.3$.  Cases 1 and 2 apply, respectively, to the shocked CNM being
optically thin and optically thick at the deceleration time.  The peak
luminosity in the optically thin case depends sensitively on $n_{18}$,
while in the optically thick regime the dependence on density is much
weaker.  We have normalized the Case 1 and Case 2 peak fluxes in
equation (\ref{eq:peakLum}) to match those derived from our numerical
results in the optically thin and thick regimes, respectively. From
equation~\eqref{eq:tslope}, we expect the luminosity to go a $L_\nu
\propto t^{-1.5}$ at late times. 
...
\label{eq:peakTime}
\end{align}
where again the normalization in Case 2 (Case 1) are chosen to match
our numerical results in the optically thick (thin) regime.  In
general, a more energetic jet produces emission which peaks later in
time.  However, the scaling of $t_p$ with $n_{18}$ is more
complicated: if the emitting region is optically thick at the
deceleration time (``Case 2''), the peak time increases with
increasing CNM density.  This is because the light curve maximum in
the optically thick case is determined by when the self-absorption
frequency passes through the observing band, which occurs later if the
CNM density is higher. Otherwise, the peak time occurs near the
deceleration time, which decreases for higher values of $n_{18}$
(eq.~\ref{eq:tdec}).

Fig.~\ref{fig:diss} shows the rough division line between Case 1 and
Case 2 for 1 and 30 GHz...{\bf AG continue}.
...
component, which is optically thick at the deceleration time (``Case
2'' in eq.~\ref{eq:peakLum}).  For high frequencies and small CNM
densities, the peak luminosity of the slow component falls off (see
right-hand panel of Fig.~\ref{fig:lightcurves}). Coincidentally, the
fast component just compensates for this decline in the slow component
luminosity, resulting in the total peak luminosity being nearly
independent of $n_{18}$ across the parameter space. In fact, the
``Case 2'' peak luminosity in equation~\ref{eq:peakLum} provides a
good approximation for the total peak luminosity for a fixed value of
$n_{18}=2000$ cm$^{-3}$.
...
for larger ambient gas densities. Note, however, that at 30 GHz, the
slow component light curve peaks later for $n_{18}=2$ cm$^{-3}$ than
for $n_{18}=60$ cm$^{-3}$. This can be understood because at high
frequencies, the emitting region is optically thin at the deceleration
time (``Case 1'' in eq.~\ref{eq:peakTime}). In this case the peak time
follows the deceleration time, which is an increasing function of the
...
close to the analytic estimate in equation~\eqref{eq:tslope}, for
$k=1$ and $p=2.3$.
...
with the jet axis (on-axis) with those observed at an angle of 0.8
radians from the jet axis (off-axis).  While at high densities
($n_{18}=2000$ cm$^{-3}$) the light curves differ little, for
$n_{18}=60$ cm$^{-3}$ the off-axis luminosity is smaller by a factor
of a few before and near the peak. However, at late times the off- and
on-axis light curves agree well. This is as expected: at late times
the jet becomes more isotropic, which means the viewing angle has
relatively little effect on the observed light curve {\bf AG note that
the slow component is relatively wide to begin with in the 2D simulation}.
...
gas density profile, $n\propto r^{-1}$, and a core galaxy profile
(equation~\ref{eq:cores}), both with $n_{18}=2$ cm$^{-3}$.  The light
curves differ by at most a factor of a few. The core and cusp light
curves are even closer at higher densities, and virtually
indistinguishable at $n_{18}=2000$ cm$^{-3}$. This is because for
larger ambient densities, the jet only samples small radii, where the
core and cusp profiles are similar (see Fig.~\ref{fig:profiles}). It
is only at lower densities, for which the Sedov radius lies outside of
the flattening of the core density profile, that noticeable
differences emerge. {\bf AG why are there differences at early times
  for $n_{18}=2$ cm$^{-3}$ but not $n_{18}=2000$ cm$^{-3}$. Is this
  simply an artifact of the starting radius?}

Fig.~\ref{fig:profs2} compares the 1D on-axis light curves for density
two density profiles: $n\propto r^{-1}$ and $n\propto r^{-1.5}$ with
$n_{18}=60$. For most times the light curves are very close.  This is
as expected, since the Sedov radius, $r_{\rm sedov}\sim 10^{18}$ cm.
{\bf BDM: combine these figures 6 and 7...}  Overall we conclude that
the radio light curve is insensitive to the precise gas density
profile, for a fixed value of the gas density near the
deceleration/Sedov radius.
...
\caption{\label{fig:diss} Fraction of the slow component ($\Gamma=2$)
  kinetic energy dissipated by the reverse shock vs. the isotropic jet
  luminosity ($L_{\rm j,iso}$) and $n_{18}$.  {\bf BDM: make y axis an
    energy axis and mark Case 1 and Case 2}}
\end{figure}
...
the optically thick limit (``Case 2'').  By contrast, for 30 GHz and
low ambient gas densities, the numerical results asymptotically
approach optically thin limit (``Case 1'').
...
side is the Case 2 peak time estimate from
...
One caveat is that we were unable to perform a 2D calculation for
$n_{18}=2$ cm$^{-3}$ to late times due to numerical issues.  Since for
...
The jet energy can still be constrained from late time
measurements. Fig.~\ref{fig:econtours}, shows a comparison between the
analytic fit for the synchrotron light curve (eq.~\ref{eq:lcAnal})
evaluated at different jet energies and radio upper limits at 5
GHz. In the top panel shows the light curves for a fixed density
normalization, whereas the bottom panel shows at every time, the
minimum of the $n_{18}=1$ cm$^{-3}$, and $n_{18}=2000$ cm$^{-3}$ light
curves. The curves in the bottom panel represent the minimum plausible
radio luminosity as a function of time for jets of different energies.
Changing the density normalization merely shifts the light curve in
time. Thus, the radio luminosity at any given time is bounded by the
luminosities for the minimum and maximum plausible $n_{18}$. An upper
limit would exclude all jet energies whose minimum possible radio
luminosity fall above it. 
...
\caption{\label{fig:econtours} {\it Top:} 5 GHz light curves and upper
  limits for different energy jets for fixed density normalization
  $n_{18}$. {\bf AG viewing angle dependence at early times?} {\it
    Bottom:} Upper limits and minimum possible radio luminosity as a
  function of time for different jet energies. Note that the radio
  luminosity at any given time is bounded by the light curves
  of the minimum plausible $n_{18}$ ($\sim 1$ cm$^{-3}$) and the
  maximum plausible $n_{18}$ ($\sim 2000$ cm$^{-3}$).}
\end{figure}
