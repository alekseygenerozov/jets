...
This is motivated by the work of \citet{Lauer+2007} who use {\it
...
Generally $0<\Gamma<1$. There are two broad classes of galaxies: cores
with $\Gamma<0.3$ and cusps with $\Gamma>0.5$. Assuming spherical
symmetry, the inner Nuker translates into a stellar density profile,
$\rho_\star\propto r^{-1-\Gamma}=r^{-\delta}$. Core galaxies will have
$1<\delta<1.3$, while cusps have $1.5<\delta<2$.

The median value of $\Gamma$ for the \citet{Lauer+2007} sample is
0.7. Thus, we adopt $\delta=1.7$ as our fiducial value {\bf Most
  expressions have $\delta=1.8$ need to fix}.
...
$r_{\rm inf}$, where $M_{\star}(r_{\rm inf})=\Mbh.$ We take $r_{\rm
  inf}=3.5 \Mbh[,7]^{0.6}$. For this $r_{\rm inf}$ and our fiducial $\delta=1.7$,
we get {\bf \bf AG: This expression is for delta=1.8 for now for
  consistency with other expressions.}

\begin{equation}
\dot{M}= 3.2 \times 10^{-6} \Mbh[,7]^{0.28} \eta_{0.02} \left(\frac{r_s}{\rm
  pc}\right) \Msun \, {\rm yr}^{-1}
\end{equation}
...
\begin{align}
  ...
  (\rs) \nonumber\\
  &n(r_s) = 0.24 \eta_{0.02} \Mbh[,7]^{-0.22} \left(\frac{r_s}{\rm
      pc}\right)^{-0.3}
\label{eq:nrs}
\end{align}
...
{\bf AG this is still left over expression from delta=1.8.}
\begin{equation}
n_{18}\simeq 0.4 \Mbh[,7]^{0.48} \eta_{0.02} v_{500}^{-1.4} {\rm
  cm^{-3}}.
\label{eq:n18}
\end{equation} 
...
population formed a Hubble time ago plus a young component that
formed a time $t_{\rm burst}$ ago and formed a fraction $f_{\rm
  burst}$ of the stars. We assume that both the young and old stars
have the same (cuspy) density profile, which implies that a gas
density profile, $n\propto r^{-1}$.

We show how $n_{18}$ varies with star burst properties ($f_{\rm
  burst}$ and $t_{\rm burst}$) in Fig.~\ref{fig:param}.  

For a stellar population formed $\sim 4\times 10^{6}$ years ago, we
obtain our maximum density of $\sim 2,700$ cm$^{-3}$. In this case
both the energy an mass budgets are dominated by fast O star winds
...
the dynamical time. For a 1 Gyr stellar population and $10^7 \Msun$
and this $r_{Ia}\simeq 10$ pc. Plugging in $10$ pc for $r_s$ is
equation~\eqref{eq:nrs}, we obtain a density of $\sim 1$
cm$^{-3}$. Taking $n\propto r^{-1}$, we then obtain $n_{18}\sim 50$ cm$^{-3}$.
...
\begin{figure} 
  ...
  \caption{\label{fig:param} Iso-contours for the density at $10^{18}$
    ...
    ago, which formed a fraction $f_{\rm burst}$ of the stars, with the rest
    ...
    burst young O stars provide substantial mass and energy
