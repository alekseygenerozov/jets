...
parameter space. For off-axis jets, the peak luminosity
would be comparable to the peak luminosity of just the slow component,
and thus would fall off for smaller ambient densities for frequencies
above 1 GHz.
...
expect that the peak luminosity will fall off roughly linearly with
$n_{18}$ at smaller densities. For example, for the derived smallest
plausible value of $n_{18}$, 0.5 cm$^{-3}$, the normalization in
equation~\eqref{eq:upLim} would increase by a factor of 5.  Radio
measurements of the peak flux following a TDE therefore serve as
...
that is consistent with it. These constraints
...
\item The slope of the gas density profile depends on the slope of the
  ...
  r^{-1.7}$. This translates into a gas density profile ranging $n
  \propto r^{-0.7}$ on large scales to $n\propto r^{-1.5}$ on very
  small scales near the SMBH.  The radio light curve of a TDE jet is
  most sensitive to the density at the deceleration/Sedov radius
  (where it has swept up its mass in CNM gas). We expect a density
  profile close $n\propto r^{-1}$ near the Sedov/deceleration radius
  of a powerful jet and thus adopt this as our fiducial density
  profile. For density profiles steeper than $n\propto r^{-1}$
  (e.g. $n\propto r^{-1.5}$) the 2D dynamical evolution of the jet
  will change in a non-trivial way, resulting in a radio light curve
  with a much steeper late time decline.
...
\item We take our two component jet model and run it through a range
  ...
  stronger function of density at higher frequencies. The time of the
  peak luminosity depends, however, on the density and can be as early
  as months and as late as one decade after the TDE. By comparing our
  ...
  of $n\propto r^{-1}$. If the density profile of most TDE hosts were
  n$\propto r^{-1.5}$ on scales of $10^{18}$ cm the existing upper
  limits would not be constraining, making it impossible to rule out
  even powerful Sw1644-like jets. Prompt follow-up in the radio band,
  as well as regular monitoring, would provide tighter constraints on
  the existence of TDE jets. Radio afterglows can serve as
  calorimeters for off-axis jets launched by TDEs, and future
