...
approaching $n\propto r^{-1.5}$, but only for radii well inside of the
stagnation radius and the SMBH's sphere of influence. The gas profile
flattens to $n\propto r^{1-\delta}$ between the stagnation radius and
the stellar break radius; however, for our fiducial value of $\delta =
1.7$, the resulting profile $n\propto r^{1-\delta} \approx r^{-0.7}$
is only moderately changed. Thus, for our fiducial profile we adopt
...
\citet{Alexander+2015} use radio observations to infer a nuclear gas
density profile of $n\propto r^{-2.6}$ for the host galaxy of the
Assasn-14li flare on scales of $\sim 10^{16}$ cm--much steeper than
...
\noindent Combining equations \eqref{eq:nrs} and (\ref{eq:profile}),
we obtain
\begin{equation}
...
curve (see dash-dotted line in Fig.~\ref{fig:cores}). . For profiles
shallower than $n\propto r^{-1}$ we do not expect a significant
difference in the late time light curve slope, but this needs to be
checked using further 2D simulations.
...
\begin{figure} 
  ...
  \caption{\label{fig:cores} {\it Top:} Comparison between on-axis
    ...
    r^{-1.5}$ ({\it dashed}) gas density profiles. Dash-dotted line
...
even using higher frequency measurements. Fig.~\ref{fig:econtours}
compares our analytic fit to the on-axis 5 GHz synchrotron light curve
...
light curve to later (earlier) times.  One implication of this
behavior is that the radio luminosity for an on-axis jet of fixed
energy cannot fall below the minimum of the largest $n_{18}$ (2000
cm$^{-3}$) and smallest $n_{18}$ (0.5 cm$^{-3}$) light curves at any
given time. For early times, the radio luminosity for an off-axis jet
propagating through low density environments would be reduced compared
to a on-axis jet, but would still generally fall above the expected
luminosity for the same energy jet propagating through a high density
medium. 
...
constraints apply to spherical outflows as well as collimated
jets. However, these constraints would only be valid if the density
profile is $n\propto r^{-1}$ or shallower.
...
\item The slope of the gas density profile depends on the slope of
  the stellar density profile. We expect a typical TDE host to have
  cuspy stellar density profile inside of a few pc, with $\rho_\star
  \propto r^{-1.7}$. This translates into a gas density profile $n
  \propto r^{(-0.7)-(-1)}$ on large scales.  The radio light curve of
  a TDE jet is most sensitive to the density at the deceleration/Sedov
  radius (where it has swept up its mass in CNM gas). For density
  profiles steeper than $n\propto r^{-1}$ (e.g. $n\propto r^{-1.5}$)
  the 2D dynamical evolution of the jet will change in a non-trivial
  way, resulting in a radio light curve with a much steeper late time
  decline. For profiles shallower than $n\propto r^{-1}$ we do not
  expect a significant difference in the late time light curve slope,
  but this needs to be checked using further 2D simulations.
...
\item We take our two component jet model and run it through a range
  ...
  cannot have jets as powerful as SwJ1644, at least for a CNM density
  profile of $n\propto r^{-1}$. If the density profile of most TDE
  hosts were n$\propto r^{-1.5}$ on scales of $10^{18}$ cm the
  existing upper limits would not be very constraining, making it
  impossible to rule out even power Sw1644-like jets. Prompt follow-up
  in the radio band, as well as regular monitoring, would provide
  tighter constraints on the existence of TDE jets. Radio afterglows
  can serve as calorimeters for off-axis jets launched by TDEs, and
  future observational efforts that capture the peak radio flux in
  thermally detected TDEs will add to the diversity of jet energies
  observed in TDE flares.  The broad range of energies (both
