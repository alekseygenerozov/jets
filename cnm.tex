...
\section{Jet Models}
\label{sec:numerical}
We calculate radio light curves due to the jet propagating through and
shocking the CNM across the physically plausible range of nuclear gas
densities.  The jet-CNM interaction simulated in one and
two-dimensions, and the resulting synchrotron emission is computed
using the procedure described in \citet{Mimica+2015}.  For the jet
model we take the preferred two-component model for SwJ1644 from
\citet{Mimica+2015}: a fast, $\Gamma=10$ inner core and a slow,
$\Gamma=2$ outer sheath.


We modify the geometry of the slow sheath for our 1D simulations to
better reproduce the 2D results. As in \citet{Mimica+2015}, for our 2D
simulations the fast inner core spans initially spans an angular
interval $0-0.1\ {\rm radians}$, while the slow outer sheath extends
from $0.1\ {\rm radians}$ to $0.5\ {\rm rad}$.  Although the sheath is
injected in a relatively narrow angular interval, at late stages of
the jet evolution the bow shock created by its interaction with CNM
spans a much larger interval in 2D simulations i.e. the slow component
becomes almost isotropic \citep[bottom two panels in Fig. 8
in][]{Mimica+2015}. To account for this, we take a slow component
extending from 0.1 to $\pi/2$ radians for our 1D simulations. The
resulting light curves better match the results from the 2D jet
simulation (see $\S~\ref{sec:1d2d}$). The ratio of the true energy of
the fast component is fixed to be 4\% of the energy in the slow
component, as in \citet{Mimica+2015}.
