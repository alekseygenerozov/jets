...
is bracketed by this range {\bf AG rephrased}.
...
We show example steady state gas density profiles for two different
stellar density profiles Fig.~\ref{fig:profiles}. The solid (dashed)
line corresponds to a cusp (core) profile with $\rho_\star \propto r^{-1.8
  (-1.1)}$ for $r < 25$ pc. Both stellar density profiles steepen at 25
pc, so that $\rho_\star \propto r^{-3}$ for $r > 25$ pc. Note that the 
mass source function is proportional to the stellar density profile
see~\ref{sec:model}.

For the cusp profile the gas density $\rho \propto r^{-1}-r^{-1.1}$
for $r < 25$ pc.  The core profile is similar to the cusp galaxy for
$r<10^{18}$ cm. It flattens near $10^{18}$ cm and then steepens again
at the break radius of 25 pc. Note that the radius where the core
profile flattens corresponds to the stagnation radius, $r_s$, where
the gas velocity goes through 0.  $r_s$ will depend on the model
parameter ($\Mbh$ and $v_w$--see GSM15).
...
in CNM gas.  The jet mostly decelerates inside of this radius, and the
density at larger radii will only affect the radio at late times after
the peak of the radio light curve {\bf note take $\gamma_j$=2 in
  calculating Sedov radius.}
...
lies inside of $r=1$ pc for $E_{54} n_{18}^{-1} < 0.2$. Thus for
...
\begin{equation}
r_{\rm sedov} = E_{54}^{1/2} \Gamma_{10}^{-1/2} n_{\rm 18}^{-1}\,{\rm pc}, 
\end{equation}
