...
post-processed to produce radio synchrotron light curves. We present
numerical light curves for a range of jet energies, ambient gas
densities and observer viewing angles $\S\ref{sec:numResults}$. In
$\S\ref{sec:param}$, we show the dependence of the peak luminosity and
time with physical parameters can be understood with a simple,
analytic blast wave model from \citet{Leventis+2012}. This model is
summarized in $\S\ref{sec:analyt}$ and
Appendix~\ref{app:analyt}. Finally, we compare our light curve results
to existing radio upper limits for TDE candidates, and show that they cannot
have jets as powerful as Sw1644. We summarize our conclusions in
$\S\ref{sec:conc}$.
...
\citep{Genzel+2003} with a top heavy IMF \citep{Bartko+2003}.  The
...
deceleration time, the peak time increases with increasing CNM
density.  This is because the light curve maximum in the optically
thick case is determined by when the self-absorption frequency passes
through the observing band, which occurs later if the CNM density is
higher. Otherwise, the peak time occurs near the deceleration time,
which decreases for higher values of $n_{18}$ (eq.~\ref{eq:tdec}).
...
\caption{\label{fig:diss} Black lines fraction of the slow component
  ($\Gamma=2$) kinetic energy dissipated by the reverse shock vs. the
  jet energy ($E_{\rm j}$) and $n_{18}$. The jet is nearly isotropic
  so that $E_{\rm j}\approx E_{\rm j, iso}$, and that the jet has
  constant luminosity for $5\times 10^5$ seconds. Red squares indicate
  the on-axis jet simulations we have performed. Blue lines show where
  the slow component of the jet optically thin/thick at the
  deceleration time.}
\end{figure}
...
for $n_{18}=60$ cm$^{-3}$. This is because at high
frequencies the emitting region is optically thin at the deceleration
time . In this case the peak time follows the deceleration time, which
is an increasing function of the ambient gas density (see
eq.~\ref{eq:tdec}).
...
\begin{figure*} 
  ...
  \caption{\label{fig:lightcurves} \textit{Left-hand panel:} On-axis
    ...
    at 5.0 GHz (from \citealt{van-Velzen+2013}).  These limits are
...
\caption{\label{fig:hist} Histogram of maximum plausible jet energies
  ...
  detections (ASSASN-14li, CSS100217, SwJ1644, and SwJ2058--{\it
    red}). The radio upper limits are taken from the compilation in
  Table 1 of \citet{Mimica+2015}, to which we add the radio upper
  limit for the PTF-09axc flare from \citet{Arcavi+2014}. Energy
  constraints for individual events are summarized in
  Table~\ref{tab:enConstr}}
\end{figure}

\begin{table}
  \caption{\label{tab:enConstr} Summary of inferred jet/outflow energies
    and upper bounds from radio detections and upper limits of optical/UV
...
\item We take a jet model which fits the radio data for the SwJ1644
  ...
  optical/UV and soft x-ray selected TDE, we show that they cannot
  have jets as powerful as SwJ1644. Prompt follow-up in the radio
  could provide tighter constraints on the existence of TDE jets.
\end{enumerate}
