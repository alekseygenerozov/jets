...
late time slope (see dash-dotted line in Fig.~\ref{fig:cores}). For
profiles shallower than $n\propto r^{-1}$ we do not expect a
significant difference in the late time light curve slope, but this
needs to be checked using further 2D simulations.
...
to an on-axis jet of the same energy.  However, their luminosities
would match at late times. 
...
maximum jet energy consistent with it.  Note that these
...
\item We take our two component jet model and run it through a range
  ...
  impossible to rule out even powerful Sw1644-like jets. Prompt follow-up
