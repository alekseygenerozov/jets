...
radius of the jet was approximately $\sim 10^{18}$
...
% The stagnation radius can be calculated analytically from the heating
% rate ($v_w$) and black hole mass ($M$)

% \begin{equation}
% r_s=f \frac{G M}{v_w^2}. 
% \label{eq:rs}
% \end{equation}


The density at the stagnation radius, $n(\rs)$ depends on the
injection rate of stellar wind material inside of $\rs$,
$\dot{M}$. This is proportional to the stellar mass enclosed inside of
$\rs$, $M_{\rm enc}(\rs)$:

\begin{equation}
\dot{M}=\frac{\eta M_{\rm enc}(\rs)}{t_h}
\label{eq:dotM}
\end{equation}

Then, the density at the stagnation radius, $n(\rs)$, can be estimated
...
\begin{equation}
\frac{4 \pi}{3} \rs^3 \mu m_p n(r_s) \approx \dot{M} t_{\rm ff} (\rs)
\label{eq:nrs}
\end{equation}
...
proportional to the stellar density (see~\ref{sec:model}). At small
radii of interest, inside the Nuker `break' radius, the stellar
density follows $\rho_\star\sim r^{-\delta}$, where $\delta = 1-2$
is the inner Nuker profile. There are two broad categories of stellar
density profiles: ``cusps'', with power law slopes $>$ 0.5 and
``cores'', with power law slopes $<0.3$ \citep{Lauer+2007}.
...
Fig.~\ref{fig:profiles} shows steady state gas density profiles for a
cusp and a core stellar density profile. For the cusp (core) profile
we take a stellar density profile $\rho_\star \propto r^{-1.8
  (-1.1)}$, for $r<25$ pc (the break radius). Both profiles have
$\rho_\star \propto r^{-3}$ outside of the break radius. 

{\bf Use cusp stellar density profile $r^{-1.7}.$ (currently
  $r^{-1.8}$.}

We mark the stagnation radius (where the velocity goes to zero) with a
blue dot on each profile. As described in \citet{Generozov+2015}, we
find that the stagnation radius can be written down in terms of the
black hole mass, $M$, the heating rate, $v_w$, and an order unity
function of the stellar density slope $\delta$, $f(\delta)$,

\begin{equation}
r_s= f(\delta) \frac{G M}{v_w^2}.
\label{eq:rs}
\end{equation}
...
the gas density profile is $\sim (4\delta-1)/6$. The median $\delta$
for low mass black holes ($\lsim 10^8 \Msun$) in the
\citet{Lauer+2007} sample is $\sim 1.7$, which gives $n(r)\propto
r^{-1}$ near $\rs$. Also, the density profile remains close to
$n\propto r^{-1}$ for the relevant range of radii for such a cuspy
stellar density\footnote{For cusp galaxies, the gas velocity
  asymptotes to a constant between the stagnation radius and break
  radius (if they are sufficiently far apart). In this region we
  expect $n\propto r^{1-\delta}$.}  Thus, we adopt $n(r)= n_{18}
(r/10^{18} {\rm cm})^{-1}$, as our fiducial density profile, and
$\delta=1.7$ as our fiducial stellar density slope.  Combining
equations~\eqref{eq:dotM}, ~\eqref{eq:nrs}, and ~\eqref{eq:rs}, and
taking $n\propto r^{-1}$, we obtain

{\bf AG this is still left over expression from delta=1.8. Need to
  explain what we assume for the influence radius.}
\begin{equation}
n_{18}\simeq 0.4 \Mbh[,7]^{0.5} \eta_{0.02} v_{500}^{-1.5} {\rm
  cm^{-3}},
\label{eq:n18}
...
where $\Mbh[,7]= \Mbh/10^7 \Msun$, $\eta_{0.02}= \eta/0.02$, and
$v_{500}=v_w/500$ km s$^{-1}$. 
...
% For the cusp profile in Fig.~\ref{fig:profiles} the gas density $\rho
% \propto r^{-0.8}-r^{-1.1}$ inside of break in stellar density (25 pc).
% The core profile is similar to the cusp galaxy for $r<10^{18}$
% cm. However, it flattens near $10^{18}$ cm.  The radius where the core
% profile flattens corresponds to the stagnation radius, $r_s$, where
% the gas velocity goes through 0.  $\rs$ depends on the model parameter
% ($\Mbh$ and $v_w$--see GSM15).



%Note that for the cusp profile with $\delta=1.8$, 
...
As the density of CNM gas increases, the rate of radiative cooling
also increases. Eventually the cooling rate will equal the heating
rate of the gas, resulting in a thermal instability. Portions of the
CNM will condense into cold clouds, eventually resulting in star
formation. This allows the CNM to regulate itself to a condition of
(marginal) thermal stability\footnote{This argument has been applied
  to larger scales in galaxies \citep{Voit+2015}}. We obtain an upper
limit for the CNM gas density by requiring it to be thermally stable
{\bf AG what do our empirical constraints on the gas density, say
  about this argument? The large Eddington ratios of most galaxies
  suggest very high densities in most galaxies...}

Defining thermally instability as occuring when the cooling time of
the gas is less than 10 times the local dynamical timescale
(\citealt{McCourt+2012}), GSM15 show that the CNM is thermally
unstable for wind heating rates below a critical value
\begin{equation}
...
The condition places an upper limit on the gas
density
\begin{equation}
...
\begin{figure} 
  ...
  \caption{\label{fig:param} Iso-contours for the density at $10^{18}$
    cm ($\mathrm{n_{18}}$) (gray lines) for a $10^{7} \Msun$ black
    hole and bursts of star formation occurring time, $t_{\rm burst}$
    ago, which formed a fraction $f$ of the stars, with the rest
    forming a Hubble time ago. Note that we assume that the density of
    old stars follows the density of young stars.  For large, recent
    burst young O\&B stars provide substantial mass and energy
