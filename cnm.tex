...
\caption{\label{fig:profiles} Steady-state gas density profiles for
  ...
  s$^{-1}$, and a cuspy ({\it solid}) or core-like ({\it dashed})
  stellar density profile. Note that we normalize the gas density to
  the density at $10^{18}$ cm ($n_{18}$). For each radius, we indicate
  the value of isotropic equivalent jet energy $E_{52} n_{18}^{1}$,
  of a $\gamma$=2 jet that sweeps up its own mass at that radius.}
\end{figure}
...
density at $10^{18}$ cm, $n_{18}$, can be calculated by using
...
To above estimates correspond to the total gas density
...
cooler, with temperature $\sim 10^{3}$ K, and would condense into
...
\begin{figure} 
  ...
  \caption{\label{fig:param} Isocontours for the density at $10^{18}$
    cm (blue lines) for a $10^{7} \, \Msun$ black
    ...
    which means that the gas density goes as $r^{-1}$.
...
velocity, $v_{\rm ff} = (2 G \Mbh/r)^{1/2}$. Then, 
...
\citet{Beloborodov&Uhm2006}:\footnote{As pointed out by
  ...
  expression could be problematic.}
...
to find the radius and the Lorentz factor of the shocked material when
the reverse shock crosses the trailing edge of the jet using
equation~\eqref{eq:gammaShGen}.
