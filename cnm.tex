...
The radio emission from a jet is most sensitive to the density of
...
profile, $n= n_0 \left(r/r_0\right)^{-k}$,

\begin{equation}
  r_{\rm sed} = r_0 \left( \frac{E(3-k)}{4\pi n_{0}
      m_{\rm p} c^2 r_0^3} \right)^{1/(3-k)}, 
  \label{eq:rdec}
\end{equation}
%
where $E$ is the isotropic equivalent energy. The jet decelerates to
non-relativistic speeds at this radius, so the Lorentz factor of the
jet has been set to 1.  Note that the jet will begin to decelerate
before reaching the Sedov radius. Specifically, the jet will
slow down once it has swept a fraction $1/\Gamma$ of its rest mass,
at the deceleration radius
...
\end{equation}
%
In the lab frame, the jet reaches the Sedov and deceleration radii at
times

\begin{align}
 t_{\rm sed}= &c/r_{\rm sed} \nonumber\\
 \approx & \left(\frac{E}{10^{52} {\rm erg}}\right)^{1/(3-k)}
 n_0^{-1/(3-k)} {\rm year}\\
 t_{\rm dec}= &c/r_{\rm dec} \nonumber\\
 \approx & \left(\frac{E}{10^{52} {\rm erg}}\right)^{1/(3-k)}
 n_0^{-1/(3-k)} \Gamma^{-2/(3-k)} {\rm year}
 \label{eq:tdec}
\end{align}
%
For our fiducial CNM density profile
$n=n_{18}\left(r/10^{18}\right)^{-1} {\rm cm}$, the Sedov radius is
given by 

\begin{equation}
r_{\rm sed} = 3 E_{54}^{1/2} n_{\rm 18}^{-1/2}\,{\rm pc}. 
\end{equation}
%
where $E_{54}=E/(10^{54} {\rm erg})$.  For a powerful SwJ1644-like jet
the deceleration radius is of order a parsec. We therefore focus on
determining the gas density on these scales. 
...
The majority of the host galaxies of observed TDE flares show evidence
of star formation within the last Gyr \citep{French+2016}, this
corresponds to the right hand side of Fig.~\ref{fig:param}. In this
region of parameter space, gas heating rate is dominated by SN Ia and
we expect $n_{18}\sim 10-100$ cm$^{-3}$. {\bf AG note that the
...
% greater than $\nu_m$. 
The peak of the light curve will occur at the deceleration time if the
self-absorption frequency is below the observing band then (``Case
1''). Otherwise, it will occur after the deceleration time, when the
self-absorption frequency crosses through the observing band (``Case
2''). The peak time for these two cases is
