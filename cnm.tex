...
off-axis jet would match that of an on-axis jet at late times. Thus,
for times after peak each light curve gives smallest plausible radio
luminosity for the corresponding jet energy. As the upper limits are
all taken at late times, the minimum curve which passes through each
upper limit corresponds to the maximum jet energy consistent with it.
Note that these constraints apply to spherical outflows as well as
collimated jets. One caveat to keep in mind is that these constraints
would only be valid if the density profile is $n\propto r^{-1}$ or
shallower.
