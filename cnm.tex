...
The median value of $\Gamma$ for low mass ($\Mbh \lsim 10^{8} \Msun$ in
the \citet{Lauer+2007} sample is 0.7 {\bf AG Also close to the rate
  weighted average in Stone \& Metzger 2016}. Thus, we adopt $\delta=1.7$ as
our fiducial value {\bf Recently changed from $\delta=1.8$, need to
...
calculated for example core and cusp stellar density profiles. We mark the
...
To specify $M_{\star}$ we need to specify the influence radius,
$r_{\rm inf}$, where $M_{\star}(r_{\rm inf})=\Mbh.$ We take $r_{\rm
  inf}=3.5 \Mbh[,7]^{0.6}$. Then for our fiducial $\delta=1.7$,
...
To obtain $n_{18}$ from $n(r_s)$, we must extrapolate. In the vicinity
of the stagnation radius, we find that the slope of the gas density
profile is $\sim (4\delta-1)/6$. For our fiducial $\delta$, 1.7, this
gives $n(r)\propto r^{-1}$ near $\rs$. The density profile remains
close to $n\propto r^{-1}$ for small radii of interest. \footnote{For
  cusp galaxies, the gas velocity asymptotes to a constant between the
  stagnation radius and break radius (if they are sufficiently far
  apart). In this region we expect $n\propto r^{1-\delta}$.}  Thus, we
adopt $n(r)= n_{18} (r/10^{18} {\rm cm})^{-1}$.  Then from
equation~\eqref{eq:nrs2}, we obtain
...
of this radius, becoming Newtonian. Thus, the profile outside of this
radius will be of minimal importance in shaping the radio
light-curve. We plot the Sedov radius for different energy jets on
each of the profiles in Fig.~\ref{fig:profiles}. Typically Nuker break
radii are larger than 10 pc. This is larger than the Sedov radius for
even a very powerful jet, with isotropic equivalent energy $E=4\times
10^{54}$ erg, interacting with a low density CNM ($n_{18}=2$). If a
nuclear star cluster (NSC) is present in the galactic center there will be a
smaller effective break radius at the outer edge of the NSC (typically
1-5 pc--see \citealt{Georgiev+2014})

Even for a break radius of 1 pc the small densities ($n_{18}<20$) and
powerful jets {\bf AG still feel that this is a relevant region of
  parameter space}. Henceforward we will not consider the effects of
the break radius in our analysis.
...
For a stellar population formed $\sim 4\times 10^{6}$ years ago, we
obtain our maximum density of $\sim 2,700$ cm$^{-3}$. In this case
both the energy an mass budgets are dominated by fast O star
winds\footnote{Type II Supernovae are another important source of
  energy for young stellar populations. Overall the energy injected by
  O stars winds and Type II SNe will be comparable
  \citep{Voss+2009}. However, heating from Type II SNe will be
  intermittent, and is not included here for simplicity.}.  Although
such a large gas density would be present in the immediate aftermath
of a starburst, the wind mass loss rate (and hence the gas density)
declines as $\eta \sim t^{-3}$. Thus we expect the gas density would
decline by an order of magnitude after just a few Myr.

The lowest density, $n_{18}\sim 0.06$ cm$^{-3}$ achieved for a
relatively modest burst, forming $f_{\rm burst}=4\times 10^{-4}$ of the
stars. In this case we have a high rating due to young O star winds
and a low mass injection rate with both the old stellar population and
O stars contributing.

In deriving this lower limit on the density, we assume that the O
stars provide a spatially continuous heating source, even on small
scales. If this were true we would expect the O heating winds to expel
gas outside of $r_s \sim 5\times 10^{16}$ cm. However, for $f_{\rm
  burst}=4\times 10^{-4}$, we expect less than one O star inside of
...
Fig.~\ref{fig:param} {\bf AG double check...}) However, we expect that
the range of hot phase densities derived above, $n_{18}\sim 1 - 1000$
cm$^{-3}$ is robust to these considerations.
...
\begin{figure} 
  ...
  \caption{\label{fig:param} Iso-contours for the density at $10^{18}$
    ...
    black hole and bursts of star formation occurring time, $t_{\rm
      burst}$, ago, which formed a fraction $f_{\rm burst}$ of the
    stars, with the rest of the stars formed a Hubble time ago. Note
    that we assume that the old and young stars have the same density
    profile. For young bursts of star formation ($t_{\rm burst}\lsim
    10^7$ yr), massive stars ($\gsim 20 \Msun$) can be a substantial
    heating source. In this case, discreteness effects not captured by
    our formalism will become important. Hatched areas indicate
    regions of parameter space, where massive stars dominate the gas
    heating rate, but we would expect less than one (doubly hatched)
    or less than ten (singly hatched) inside the nominal stagnation
    radius (equation~\eqref{eq:rs})}
\end{figure}
