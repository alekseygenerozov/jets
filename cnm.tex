...
Our formalism give a somewhat lower value for the density than the
observed value. In order to precisely model the gas density
distribution in a particular galactic nucleus, detailed knowledge of
the stellar population and its distribution are necessary (see
e.g.~\citealt{Cuadra+2008}). In the case of the Galactic Center the young
stars are more centrally concentrated than we assume in our
model...{\bf AG double check--see Ghenzel+2003. Describe density in
  case young stars dominate the central regions.}
...
values for power law slopes of the gas density profile, $k=1$, and
energy distribution of accelerated electrons, $p=2.3$, gives
...
time, which is larger for smaller ambient gas densities (see
eq.~\ref{eq:tdec}). 

\subsubsection{Numerical results}
The left-hand panel of Fig.~\ref{fig:lightcurves} shows example light
curves from both 1D and 2D simulations for different energy jets and
density normalizations. The peak luminosity is virtually independent
of density.  This is as expected for high densities and low
frequencies: the emission is dominated by the slow component, which is
optically thick at the deceleration time. In this case the peak
luminosity is virtually independent of density (``Case 2'' in
