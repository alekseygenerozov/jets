...
corresponds to the minimum possible CNM density: $\simeq 0.1$ cm$^{-3}$.

However, given the above we would expect less than 1 massive star
within the stagnation radius. In this case our formalism would break
down, and the true stagnation radius would be at larger radii implying
a larger density in this scenario.
...
equality between the forward and reverse shocks\footnote{but see
  \citet{Beloborodov&Uhm2006}}. In the ultra-relativistic limit
...
(see Figure 3 of \citealt{Mimica+2015}). {\bf AG this is stupid--the
  profiles are similar inside of the deceleration radius, so we would
  not expect the profiles to be different.}
