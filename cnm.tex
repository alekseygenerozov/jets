...
large scale inflow rate $\dot{M}$. So equation~\eqref{eq:tgrow}
This can be translated into an upper bound for gas density, $n_{18}$,
if we assume an inflow velocity $\sqrt{G M/r}$:

\begin{align}
n_{18} &\lsim 10^4 \Mbh[,7]^{0.5} \left(\frac{f_{\rm in}}{0.01}\right)
{\rm cm^{-3}}, 
\label{eq:tgrowDens}
\end{align}

e.g. within a factor of a few of the high density limit given in
equation~\eqref{eq:n18max}, for $f_{\rm in}$=0.01. However, we note
